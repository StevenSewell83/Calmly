...
export function AIUsagePanel() {
  ...
  useEffect(() => {
    void window.calmly.ai.getUsage().then((r) => {
      ...
      {suspended && (
        <div className="flex items-center gap-2 mb-4 px-3 py-2.5 rounded-2xl bg-amber-50 border border-amber-200">
          ...
          <p className="text-xs text-amber-700">
            ...
            {new Date(suspendedUntilMs).toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" })}.
          </p>
        </div>
      )}
          ...
          {Object.keys(usage.byPromptClass).length > 0 && (
            <div className="border-t border-stone-100 pt-3 flex flex-col gap-1.5">
              {Object.entries(usage.byPromptClass).map(([cls, data]) => (
                <div key={cls} className="flex items-center gap-3 text-xs text-stone-600">
                  <span className="flex-1 truncate">{ACTION_LABELS[cls] ?? cls}</span>
                  <span className="text-stone-400 tabular-nums">{data.requests}×</span>
                  <span className="text-stone-400 tabular-nums">{(data.inputTokens + data.outputTokens).toLocaleString()} tok</span>
                </div>
              ))}
            </div>
          )}
...
function Stat({ label, value }: { label: string; value: string }) {
  return (
    <div className="flex flex-col">
      <span className="text-[9px] font-bold tracking-[0.18em] uppercase text-stone-400 mb-0.5">{label}</span>
      <span className="text-lg font-medium text-stone-800 tabular-nums">{value}</span>
    </div>
  );
}
