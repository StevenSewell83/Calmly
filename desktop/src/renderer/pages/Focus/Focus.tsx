...
type MakeStartableModalState =
  ...
  | {
      ...
      task: PlanTaskItem;
      suggestionId: string;
      suggestion: MakeStartableResult;
    }
  | { kind: "error"; task: PlanTaskItem; error: AIError }
  | null;

export function Focus() {
  const { state, startFocus, endFocus, markDone, switchTask, refresh } =
    useFocusSession();
  const [adHocOpen, setAdHocOpen] = useState(false);
  const [aiEnabled, setAiEnabled] = useState(false);
  const [makeStartable, setMakeStartable] =
    useState<MakeStartableModalState>(null);
  ...
  const handleMakeStartable = useCallback(async (task: PlanTaskItem) => {
    ...
    try {
      ...
      setMakeStartable((s) =>
        s ? { kind: "error", task: s.task, error: { kind: "unknown" } } : null,
      );
    }
  }, []);
    ...
    <>
      ...
      {makeStartable && (
        <MakeStartableModal
          ...
          state={
            makeStartable.kind === "loading"
              ? { kind: "loading" }
              : makeStartable.kind === "error"
                ? { kind: "error", error: makeStartable.error }
                : {
                    kind: "done",
                    suggestionId: makeStartable.suggestionId,
                    suggestion: makeStartable.suggestion,
                  }
          }
        ...
        <button onClick={onStart} className="flex-1 text-left min-w-0">
          <div className="flex items-center gap-4">
            <div className="flex-1 min-w-0">
              <p className="text-sm font-medium text-stone-800 group-hover:text-emerald-700 truncate">
                {task.title}
              </p>
