...
export function useFocusSession() {
  ...
  const refresh = useCallback(async () => {
    ...
    const allTasks = planRes.ok
      ? [...planRes.plan.scheduled, ...planRes.plan.backlog]
      : [];
    const session = sessionRes.ok ? sessionRes.session : null;
    const task = session
      ? (allTasks.find((t) => t.id === session.task_id) ?? null)
      : null;
  ...
  const startFocus = async (taskId: string) => {
    const source =
      state.todayTasks.find((t) => t.id === taskId)?.scheduled_start !== null
        ? ("scheduled" as const)
        : ("ad-hoc" as const);
