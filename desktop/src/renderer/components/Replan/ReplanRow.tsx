...
export function ReplanRow({ task, selected, onSelect, onAction, busy }: Props) {
  ...
  return (
    <li className={`flex items-start gap-3 p-3 rounded-2xl border transition-colors ${selected ? "bg-emerald-50 border-emerald-200" : "bg-white border-stone-100"}`}>
      <input
        type="checkbox"
        checked={selected}
      ...
      <div className="flex-1 min-w-0 space-y-2">
        ...
        <div className="flex flex-wrap gap-1.5">
          {/* Push */}
          {task.scheduled_start !== null && PUSH_PRESETS.map(({ label, ms }) => (
            <button key={label} disabled={busy}
              onClick={() => onAction({ type: "push", taskId: task.id, offsetMs: ms })}
              className="text-[10px] px-2 py-0.5 rounded-lg bg-stone-100 hover:bg-stone-200 text-stone-600 disabled:opacity-40"
            >{label}</button>
          ))}
          {/* Shrink */}
          {task.scheduled_start !== null && durationMins !== null && SHRINK_PRESETS.filter((p) => Math.round(p.ms / 60_000) < durationMins).map(({ label, ms }) => (
            <button key={label} disabled={busy}
              onClick={() => onAction({ type: "shrink", taskId: task.id, durationMs: ms })}
          ...
          {dateInput && (
            <button disabled={busy}
              onClick={() => { onAction({ type: "moveToDate", taskId: task.id, targetDayMs: new Date(dateInput + "T12:00:00").getTime() }); setDateInput(""); }}
              className="text-[10px] px-2 py-0.5 rounded-lg bg-stone-100 hover:bg-stone-200 text-stone-600 disabled:opacity-40"
            >Move</button>
          )}
        </div>
