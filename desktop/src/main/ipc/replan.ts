...
export function registerReplanIpc(): void {
  authedHandler<{ ok: true } | { ok: false; error: string }>(
    ...
    (ctx, raw) => {
      ...
      ctx.db
        .prepare(
          `INSERT INTO replan_events (id, user_id, reason, created_at) VALUES (?, ?, ?, ?)`,
        )
    ...
    (ctx, raw) => {
      ...
      for (const action of raw as unknown[]) {
        ...
        if (type === "push" && typeof action.offsetMs === "number") {
          ...
          const task = [...plan.scheduled, ...plan.backlog].find(
            (t) => t.id === taskId,
          );
          if (
          ...
          type === "moveToDate" &&
          typeof action.targetDayMs === "number"
        ) {
          const r = moveToDate(
            ctx.db,
            ctx.userId,
            taskId,
            action.targetDayMs as number,
            ctx.now,
          );
          if (r.ok) applied++;
        }
