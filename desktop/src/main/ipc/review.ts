...
export type ReviewDropResult = DropResult | { ok: false; error: "NotSignedIn" };
...
export function registerReviewIpc(): void {
  authedHandler<ReviewSummaryResult>("review:summary", (ctx) => {
    return {
      ok: true,
      summary: getReviewSummary(ctx.db, ctx.userId, ctx.now, ctx.tz),
    };
  });

  authedHandler<ReviewCarryForwardResult>("review:carryForward", (ctx, raw) => {
    if (!isObject(raw) || !isStringArray(raw.taskIds))
  ...
  authedHandler<ReviewSaveReflectionResult>(
  ...
  authedHandler<ReviewCompleteShutdownResult>(
    "review:completeShutdown",
    (ctx, raw) => {
      const text =
        isObject(raw) && typeof raw.text === "string" ? raw.text : null;
