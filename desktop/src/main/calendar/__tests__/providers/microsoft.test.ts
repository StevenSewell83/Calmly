...
const makeEv = (id: string, extra: Partial<MicrosoftEvent> = {}): MicrosoftEvent => ({
  id,
...
describe("fetchMicrosoftEvents", () => {
  ...
  it("returns events from a single page with deltaLink", async () => {
    const page = { value: [makeEv("m1"), makeEv("m2")], "@odata.deltaLink": "delta-1" };
  ...
  it("uses deltaLink URL directly on incremental sync", async () => {
    const mockFetch = makeFetch([{ value: [makeEv("m3")], "@odata.deltaLink": "delta-2" }]);
    vi.stubGlobal("fetch", mockFetch);

    await fetchMicrosoftEvents("access", W_START, W_END, "https://graph.microsoft.com/delta?$token=abc");
  ...
  it("paginates using @odata.nextLink", async () => {
    const page1 = { value: [makeEv("p1")], "@odata.nextLink": "https://graph.microsoft.com/next" };
  ...
  it("second page request uses nextLink URL", async () => {
    const mockFetch = makeFetch([
      { value: [makeEv("x1")], "@odata.nextLink": "https://graph.microsoft.com/page2" },
  ...
  it("throws MicrosoftSyncExpiredError on 410", async () => {
    ...
    await expect(fetchMicrosoftEvents("access", W_START, W_END, "stale")).rejects.toBeInstanceOf(MicrosoftSyncExpiredError);
  });

  it("throws MicrosoftSyncExpiredError when body contains SyncStateNotFound", async () => {
    vi.stubGlobal("fetch", makeErrorFetch(400, '{"error":{"code":"SyncStateNotFound"}}'));
    await expect(fetchMicrosoftEvents("access", W_START, W_END, "stale")).rejects.toBeInstanceOf(MicrosoftSyncExpiredError);
  });

  it("throws generic error on other HTTP errors", async () => {
    vi.stubGlobal("fetch", makeErrorFetch(503, "Service unavailable"));
    await expect(fetchMicrosoftEvents("access", W_START, W_END)).rejects.toThrow("503");
  });

  it("includes @removed events in output (worker handles deletion)", async () => {
    const removed: MicrosoftEvent = { ...makeEv("m-del"), "@removed": { reason: "deleted" } };
    vi.stubGlobal("fetch", makeFetch([{ value: [removed], "@odata.deltaLink": "d" }]));
...
describe("microsoftEventStartMs / microsoftEventEndMs", () => {
  it("parses UTC dateTime strings", () => {
    const e = makeEv("x");
    expect(microsoftEventStartMs(e)).toBe(new Date("2024-03-01T10:00:00").getTime());
    expect(microsoftEventEndMs(e)).toBe(new Date("2024-03-01T11:00:00").getTime());
  });
  ...
  it("parses all-day event dateTime (midnight UTC)", () => {
    ...
    expect(microsoftEventStartMs(e)).toBe(new Date("2024-03-01T00:00:00").getTime());
    expect(microsoftEventEndMs(e)).toBe(new Date("2024-03-02T00:00:00").getTime());
  });
});
