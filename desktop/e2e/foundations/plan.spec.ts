...
test.describe("CL-06-e2e · Plan view — place, reload, persist", () => {
  ...
  test("backlog shows unseeded tasks; placed block persists across reload", async () => {
    ...
    try {
      ...
      try {
        ...
        try {
          ...
          await expect(
            win2.getByRole("heading", { name: /Plan/i }),
          ).toBeVisible({
            timeout: 10_000,
          });
  ...
  test("schedule via IPC: backlog task moves to grid at 09:00, persists on reload", async () => {
    ...
    try {
      ...
      try {
        ...
        const backlog = win.getByRole("complementary", { name: "Backlog" });
        await expect(backlog.getByText(TASK_TITLE)).toBeVisible({
          timeout: 5_000,
        });
        ...
        await expect(backlog.getByText(TASK_TITLE)).not.toBeVisible({
          timeout: 5_000,
        });
      } finally {
        await secondLaunch.dispose();
